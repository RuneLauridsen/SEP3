...
public class TicTacToe implements GameLogic {
    ...
    public TicTacToe() {
        String raw = ResourcesUtil.readResourceAsString(TicTacToe.class, "/TicTacToeWinPatterns.txt");
    ...
    public String validateMove(TicTacToeData data, TicTacToeMove move, Account moveBy) {
        // TODO(rune): Re-factor. Svært at holde styre på state, når alle predicates står først -> brug else i stedet.
        // TODO(rune): Må vi gerne droppe getters og setters? Det er dumt og jeg savner '++', '--' og '='  :((((

        // Validate TicTacToeData.
        if (data.players().length != NUM_PLAYERS) return String.format("Number of players must be %d, but was %s.", NUM_PLAYERS, data.players().length);
        if (data.squares().length != NUM_SQUARES) return String.format("Number of squares must be %d, but was %s.", NUM_SQUARES, data.squares().length);
        if (data.nextTurnAccountId() != moveBy.accountId()) return String.format("Next turn account id is %d, but move by account id was %d.", data.nextTurnAccountId(), moveBy.accountId());
        ...
        if (player == null) return String.format("Next turn account id is %d, but is not in the players array.", data.nextTurnAccountId());

        // Validate TicTacToeMove.
        if (move.takeFromIndex() < 0 || move.takeFromIndex() >= NUM_SQUARES) return String.format("Take from index out of range (was %d, but must be between 0 and %d).", move.takeFromIndex(), NUM_SQUARES);
        if (move.placeOnIndex() < 0 || move.placeOnIndex() >= NUM_SQUARES) return String.format("Place on index out of range (was %d, but must be between 0 and %d).", move.placeOnIndex(), NUM_SQUARES);

        // Check valid move.
        if (data.squares()[move.placeOnIndex()] != ' ') return "Square is already occupied.";
        if (data.squares()[move.takeFromIndex()] != player.team() && player.remainingPieces() <= 0) return "Cannot take from square.";
